//strict_using_st_wt
__global__ void hash_search_nvmb(
		selem_t			*in,
		loc_t			*out,
		bucket_t		*hash_table,
		int				total_elem_num,
		int				thread_num)
{
	...
	for (id = elem_id; id < total_elem_num; id += step) {
		...
		if (b->sig[simd_lane] == elem->sig) {
			...
			asm("st.global.wt.u32 [%0], %1;" :: "l"(&out[id << 1]), "r"(b->loc[simd_lane]) : "memory");
			MEM_FENCE;
		}
		...
		if (b->sig[simd_lane] == elem->sig) {
			...
			asm("st.global.wt.u32 [%0], %1;" :: "l"(&out[(id << 1) + 1]), "r"(b->loc[simd_lane]) : "memory");
			MEM_FENCE;
		}
	}
	return;
