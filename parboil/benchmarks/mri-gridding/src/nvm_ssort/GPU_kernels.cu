...
__global__ void binning_kernel_nvmb (unsigned int n, ReconstructionSample* sample_g, unsigned int* idxKey_g,
                                unsigned int* idxValue_g, unsigned int* binCount_g, unsigned int binsize, unsigned int gridNumElems){
  unsigned int key;
  unsigned int sampleIdx = blockIdx.x*blockDim.x+threadIdx.x;
  ReconstructionSample pt;
  ...
  if (sampleIdx < n){
    ...
    //idxKey_g[sampleIdx] = key;
    //idxValue_g[sampleIdx] = sampleIdx;
    ST_WT_INT(&idxKey_g[sampleIdx], key);
    MEM_FENCE;
    ST_WT_INT(&idxValue_g[sampleIdx], sampleIdx);
    MEM_FENCE;
  }
}

__global__ void binning_kernel_nvmd (unsigned int n, ReconstructionSample* sample_g, unsigned int* idxKey_g,
                                unsigned int* idxValue_g, unsigned int* binCount_g, unsigned int binsize, unsigned int gridNumElems){
  unsigned int key;
  unsigned int sampleIdx = blockIdx.x*blockDim.x+threadIdx.x;
  ReconstructionSample pt;
...
__global__ void reorder_kernel_nvma(int n, unsigned int* idxValue_g, ReconstructionSample* samples_g, ReconstructionSample* sortedSample_g){
  unsigned int index = blockIdx.x*blockDim.x + threadIdx.x;
  unsigned int old_index;
  ReconstructionSample pt;
  ...
  if (index < n){
    ...
    pt = samples_g[old_index];
    //    sortedSample_g[index] = pt;
    float *p = (float *)sortedSample_g + index*6;
    ST_WB_FLOAT(p, pt.real);
    ST_WB_FLOAT(p+1, pt.imag);
    ST_WB_FLOAT(p+2, pt.kX);
    ST_WB_FLOAT(p+3, pt.kY);
    ST_WB_FLOAT(p+4, pt.kZ);
    ST_WB_FLOAT(p+5, pt.sdc);
  }
}

__global__ void reorder_kernel_nvmb(int n, unsigned int* idxValue_g, ReconstructionSample* samples_g, ReconstructionSample* sortedSample_g){
  unsigned int index = blockIdx.x*blockDim.x + threadIdx.x;
  unsigned int old_index;
  ReconstructionSample pt;
  ...
  if (index < n){
    ...
    //sortedSample_g[index] = pt;
    float *p = (float *)sortedSample_g + index*6;
    ST_WT_FLOAT(p, pt.real);
    MEM_FENCE;
    ST_WT_FLOAT(p+1, pt.imag);
    MEM_FENCE;
    ST_WT_FLOAT(p+2, pt.kX);
    MEM_FENCE;
    ST_WT_FLOAT(p+3, pt.kY);
    MEM_FENCE;
    ST_WT_FLOAT(p+4, pt.kZ);
    MEM_FENCE;
    ST_WT_FLOAT(p+5, pt.sdc);
    MEM_FENCE;
  }
}

__global__ void reorder_kernel_nvmd(int n, unsigned int* idxValue_g, ReconstructionSample* samples_g, ReconstructionSample* sortedSample_g){
  unsigned int index = blockIdx.x*blockDim.x + threadIdx.x;
  unsigned int old_index;
  ReconstructionSample pt;
...
__global__ void reorder_kernel_nvmg(int n, unsigned int* idxValue_g, ReconstructionSample* samples_g, ReconstructionSample* sortedSample_g){
  unsigned int index = blockIdx.x*blockDim.x + threadIdx.x;
  unsigned int old_index;
  ReconstructionSample pt;
  ...
  if (index < n){
    float *p = (float *)sortedSample_g + index*6;
    float *plog = NVM_log + index*6;
    ST_WT_FLOAT(plog, *p);
    ST_WT_FLOAT(plog+1, *(p+1));
    ST_WT_FLOAT(plog+2, *(p+2));
    ST_WT_FLOAT(plog+3, *(p+3));
    ST_WT_FLOAT(plog+4, *(p+4));
    ST_WT_FLOAT(plog+5, *(p+5));
...
__global__ void reorder_kernel_nvmi(int n, unsigned int* idxValue_g, ReconstructionSample* samples_g, ReconstructionSample* sortedSample_g){
  unsigned int index = blockIdx.x*blockDim.x + threadIdx.x;
  unsigned int old_index;
  ReconstructionSample pt;
...
__global__ void gridding_GPU_nvma (ReconstructionSample* sample_g, unsigned int* binStartAddr_g, float2* gridData_g, float* sampleDensity_g, float beta){
  __shared__ ReconstructionSample sharedBin[TILE];
  ...
  //gridData_g[idx] = pt;
  //sampleDensity_g[idx] = density;
  float *p = (float *)&gridData_g + idx * 2;
  ST_WT_FLOAT(p, pt.x);
  MEM_FENCE;
  ST_WT_FLOAT(p+1, pt.y);
  MEM_FENCE;
  ST_WT_FLOAT(&sampleDensity_g[idx], density);
  MEM_FENCE;
}


__global__ void gridding_GPU_nvmb (ReconstructionSample* sample_g, unsigned int* binStartAddr_g, float2* gridData_g, float* sampleDensity_g, float beta){
  __shared__ ReconstructionSample sharedBin[TILE];
