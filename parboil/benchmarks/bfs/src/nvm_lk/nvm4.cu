...
struct LocalQueues_nvm4 : public LocalQueues {
...
BFS_in_GPU_kernel_nvm4(int *q1, 
		       ...
		       int *overflow) 
{
  __shared__ LocalQueues_nvm4 local_q;
  ...
  while(1){
    ...
    SFENCE; CACHEWB; SFENCE; PCOMMIT; SFENCE;
        __syncthreads();
        SET_NVM_FLAG_WB_PC(0);
      if (tid == 0) {
        ST_WB_INT(&NVM_last_iter[blockIdx.x], interation);
        CLWB(&NVM_last_iter[blockIdx.x]);
      }
