...
struct LocalQueues_nvmu : public LocalQueues {
  __device__ void reset(int index, dim3 block_dim) {
    LocalQueues::reset(index, block_dim);
    ST_WB_INT(&shadow_sharers[index], sharers[index]);
    CLWB(&shadow_sharers[index]);
  ...
  __device__ void append(int index, int *overflow, int value) {
    ...
    if (tail_index >= W_QUEUE_SIZE) {
      *overflow = 1;
      ...
      ST_WB_INT(&shadow_elems[index][tail_index], value);
      CLWB(&shadow_elems[index][tail_index]);
    }
    SFENCE; PCOMMIT; SFENCE;
  }
  __device__ int size_prefix_sum(int (&prefix_q)[NUM_BIN]) {
    prefix_q[0] = 0;
    //ST_WB_INT(&shadow_prefix_q[0], 0);
    //CLWB(&shadow_prefix_q[0]);
    //SFENCE; PCOMMIT; SFENCE;
    for(int i = 1; i < NUM_BIN; i++){
      prefix_q[i] = prefix_q[i-1] + tail[i-1];
      ST_WB_INT(&shadow_prefix_q[i],  prefix_q[i]);
      CLWB(&shadow_prefix_q[i]);
      SFENCE; PCOMMIT; SFENCE;
    }
  ...
  __device__ void concatenate_smem(int *dst, int (&prefix_q)[NUM_BIN], int *shadow) {
    ...
    while(local_shift < tail[q_i]){
      ...
      ST_WB_INT(shadow + prefix_q[q_i] + local_shift, elems[q_i][local_shift]);
      CLWB(shadow + prefix_q[q_i] + local_shift);
      SFENCE; PCOMMIT; SFENCE;
  ...
  __device__ void concatenate_gmem(int *dst, int (&prefix_q)[NUM_BIN]) {
    ...
    while(local_shift < tail[q_i]){
      //dst[prefix_q[q_i] + local_shift] = elems[q_i][local_shift];
      ST_WB_INT(&dst[prefix_q[q_i] + local_shift], elems[q_i][local_shift]);
      CLWB(&dst[prefix_q[q_i] + local_shift]);
      SFENCE; PCOMMIT; SFENCE;
      local_shift += sharers[q_i];
    }
  }
...
BFS_in_GPU_kernel_nvmu(int *q1, 
		       ...
		       int *overflow) 
{
  __shared__ LocalQueues_nvmu local_q;
