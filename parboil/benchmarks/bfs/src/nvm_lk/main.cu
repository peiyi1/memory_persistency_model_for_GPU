...
int main(int argc, char** argv) 
{
  ...
  do
  {
	...
	st.lap_start();
	if (nvm_opt == 'a')
	  BFS_in_GPU_kernel<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'b')
	  BFS_in_GPU_kernel_nvmb<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'o')
	  BFS_in_GPU_kernel_nvmo<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'u')
	  BFS_in_GPU_kernel_nvmu<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'e')
	  BFS_in_GPU_kernel_nvme<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'r')
	  BFS_in_GPU_kernel_nvmr<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
	else if (nvm_opt == 'x')
	  BFS_in_GPU_kernel_nvmx<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
        else if (nvm_opt == 's')
          BFS_in_GPU_kernel_nvms<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
        else if (nvm_opt == '3')
          BFS_in_GPU_kernel_nvm3<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
        else if (nvm_opt == '4')
          BFS_in_GPU_kernel_nvm4<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k,d_overflow);
       st.lap_end();
  ...
  st.print_avg_usec("BFS_in_GPU_kernel");
  cudaThreadSynchronize();
  pb_SwitchToTimer(&timers, pb_TimerID_COPY);
