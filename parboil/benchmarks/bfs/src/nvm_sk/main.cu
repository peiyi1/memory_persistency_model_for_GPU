...
int main(int argc, char** argv) 
{
  ...
  do
  {
	...
	st.lap_start();
	if (nvm_opt == 'a') {
	  BFS_kernel<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
	} else if (nvm_opt == 'b') {
	  BFS_kernel_nvmb<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
	} else if (nvm_opt == 'o') {
	  BFS_kernel_nvmo<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
	} else if (nvm_opt == 'u') {
	  BFS_kernel_nvmu<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
	} else if (nvm_opt == 'f') {
	  NVM_KLOG_FILL(NVM_klog, d_q2, sizeof(int) * num_of_nodes);
	  NVM_KLOG_FILL(NVM_klog, d_color, sizeof(int) * num_of_nodes);
	  NVM_KLOG_FILL(NVM_klog, d_cost, sizeof(int) * num_of_nodes);
	  NVM_KLOG_FILL(NVM_klog, tail, sizeof(int));
	  cudaDeviceSynchronize();
	  BFS_kernel<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
	} else if (nvm_opt == 'k') {
          BFS_kernel<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
          cudaDeviceSynchronize();
          kernel_l2wb <<< grid, threads>>>();
        } else if (nvm_opt == 'l') {
          BFS_kernel<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail,GRAY0,k);
          cudaDeviceSynchronize();
          kernel_l2wb_pct <<< grid, threads>>>();
        } else if (nvm_opt == 'm') {
          NVM_KLOG_FILL(NVM_klog, d_q2, sizeof(int) * num_of_nodes);
          NVM_KLOG_FILL(NVM_klog, d_color, sizeof(int) * num_of_nodes);
          NVM_KLOG_FILL(NVM_klog, d_cost, sizeof(int) * num_of_nodes);
	...
	BFS_kernel<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes, d_graph_edges, d_color, d_cost, num_t, tail, GRAY1,k);
      }
    }
    k++;
  }
