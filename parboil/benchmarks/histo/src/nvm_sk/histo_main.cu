...
__device__ void testIncrementGlobal_strict (
				     unsigned int *global_histo,
				     unsigned int sm_range_min,
				     unsigned int sm_range_max,
				     const uchar4 sm)
...
__device__ void testIncrementLocal_strict (
				    unsigned int *global_overflow,
				    unsigned int smem[KB][256],
				    const unsigned int myRange,
				    const uchar4 sm)
...
__device__ void clearMemory_strict (unsigned int smem[KB][256])
{
  for (int i = threadIdx.x; i < BINS_PER_BLOCK / 4; i += blockDim.x)
    {
      ((unsigned int*)smem)[i] = 0;
...
__device__ void copyMemory_strict (unsigned int *dst, unsigned int src[KB][256])
{
  for (int i = threadIdx.x; i < BINS_PER_BLOCK/4; i += blockDim.x)
    {
      atomicAdd(dst+i*4, (((unsigned int*)src)[i] >> 0) & 0xFF);
...
__global__ void histo_main_kernel_strict (
				   uchar4 *sm_mappings,
				   unsigned int num_elements,
				   unsigned int sm_range_min,
				   unsigned int sm_range_max,
