...
__global__ void histo_prescan_kernel_nvmb (unsigned int* input, int size, unsigned int* minmax)
{
    __shared__ float Avg[PRESCAN_THREADS];
    __shared__ float StdDev[PRESCAN_THREADS];
...
if(threadIdx.x < stride__){\
	...
	stddev /= PRESCAN_THREADS;
        ...
        // away from the average. The value 10 was obtained empirically.
	//atomicMin(minmax,((unsigned int)(avg-10*stddev))/(KB*1024));
	ST_WT_INT(minmax, ((unsigned int)(avg-10*stddev))/(KB*1024));
	MEM_FENCE;
        //atomicMax(minmax+1,((unsigned int)(avg+10*stddev))/(KB*1024));
	ST_WT_INT(minmax+1, ((unsigned int)(avg-10*stddev))/(KB*1024));
	MEM_FENCE;
    }
}

__global__ void histo_prescan_kernel_nvmo (unsigned int* input, int size, unsigned int* minmax)
{
    __shared__ float Avg[PRESCAN_THREADS];
    __shared__ float StdDev[PRESCAN_THREADS];
...
if(threadIdx.x < stride__){\
	...
	CLWB(minmax); SFENCE;
        atomicMax(minmax+1,((unsigned int)(avg+10*stddev))/(KB*1024));
	CLWB(minmax+1); SFENCE;
    }
}


__global__ void histo_prescan_kernel_nvmu (unsigned int* input, int size, unsigned int* minmax)
{
    __shared__ float Avg[PRESCAN_THREADS];
    __shared__ float StdDev[PRESCAN_THREADS];
