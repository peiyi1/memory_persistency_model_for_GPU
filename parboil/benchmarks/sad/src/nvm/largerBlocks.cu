...
__global__ void larger_sad_calc_8_nvmo(unsigned short *blk_sad,
				  int mb_width,
				  int mb_height)
{
  int tx = threadIdx.y & 1;
  ...
  for (search_pos = threadIdx.x; search_pos < (MAX_POS+1)/2; search_pos += 32)
    {
      ...
      ((uint *)bo_6)[search_pos]                  = i00 + i10;
      CLWB(&((uint *)bo_6)[search_pos]); 
      SFENCE;
      ((uint *)bo_6)[search_pos+MAX_POS_PADDED/2] = i01 + i11;
      CLWB(&((uint *)bo_6)[search_pos+MAX_POS_PADDED/2]);
      SFENCE;
      ((uint *)bo_5)[search_pos]                  = i00 + i01;
      CLWB(&((uint *)bo_5)[search_pos]);
      SFENCE;
      ((uint *)bo_5)[search_pos+2*MAX_POS_PADDED/2] = i10 + i11;
      CLWB(&((uint *)bo_5)[search_pos+2*MAX_POS_PADDED/2]);
      SFENCE;
      ((uint *)bo_4)[search_pos]                  = (i00 + i01) + (i10 + i11);
      CLWB(&((uint *)bo_4)[search_pos]);
      SFENCE;
    }
}

__global__ void larger_sad_calc_8_nvmu(unsigned short *blk_sad,
				  int mb_width,
				  int mb_height)
{
  int tx = threadIdx.y & 1;
...
__global__ void larger_sad_calc_8_nvmb(unsigned short *blk_sad,
				  int mb_width,
				  int mb_height)
{
  int tx = threadIdx.y & 1;
  ...
  for (search_pos = threadIdx.x; search_pos < (MAX_POS+1)/2; search_pos += 32)
    {
      ...
      ((uint *)bo_4)[search_pos]                  = (i00 + i01) + (i10 + i11);
      */
      ST_WT_INT(&(((uint *)bo_6)[search_pos]), i00 + i10);
      MEM_FENCE;
      ST_WT_INT(&(((uint *)bo_6)[search_pos+MAX_POS_PADDED/2]), i01 + i11);
      MEM_FENCE;
      ST_WT_INT(&(((uint *)bo_5)[search_pos]), i00 + i01);
      MEM_FENCE;
      ST_WT_INT(&(((uint *)bo_5)[search_pos+2*MAX_POS_PADDED/2]), i10 + i11);
      MEM_FENCE;
      ST_WT_INT(&(((uint *)bo_4)[search_pos]), (i00 + i01) + (i10 + i11));
      MEM_FENCE;
    }
}
