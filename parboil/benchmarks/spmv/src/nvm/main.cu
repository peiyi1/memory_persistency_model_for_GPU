...
int main(int argc, char** argv) {
  ...
  for (int i= 0; i<1; i++) {
    ...
    if (nvm_opt == 'a') {
      ...
      spmv_jds_nvmb<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == 'g') {
      spmv_jds_nvmg<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == 'i') {
      spmv_jds_nvmi<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == 'o') {
      spmv_jds_nvmo<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == 'u') {
      spmv_jds_nvmu<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '1') {
      spmv_jds_nvm1<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '2') {
      spmv_jds_nvm2<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '3') {
      spmv_jds_nvm3<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '4') {
      spmv_jds_nvm4<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '5') {
      spmv_jds_nvm5<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    } else if (nvm_opt == '6') {
      spmv_jds_nvm6<<<grid, block>>>(d_Ax_vector,
				d_data,d_indices,d_perm,
				d_x_vector,d_nzcnt,dim);
    }
    st.lap_end();
  ...
  st.print_avg_usec("spmv_jds", grid);
  cudaThreadSynchronize();
