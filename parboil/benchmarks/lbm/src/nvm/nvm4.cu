...
__global__ void performStreamCollide_kernel_nvm4( float* srcGrid, float* dstGrid ) 
{
  SWEEP_VAR
    SWEEP_X = threadIdx.x;
  SWEEP_Y = blockIdx.x;
  ...
  // Create undo logs
  ST_WB_FLOAT(&DST_C(NVM_log), tempC);
  ST_WB_FLOAT(&DST_N(NVM_log), tempN);
  ST_WB_FLOAT(&DST_S(NVM_log), tempS);
  ST_WB_FLOAT(&DST_E(NVM_log), tempE);
  ST_WB_FLOAT(&DST_W(NVM_log), tempW);
  ST_WB_FLOAT(&DST_T(NVM_log), tempT);
  ST_WB_FLOAT(&DST_B(NVM_log), tempB);
  ST_WB_FLOAT(&DST_NE(NVM_log), tempNE);
  ST_WB_FLOAT(&DST_NW(NVM_log), tempNW);
  ST_WB_FLOAT(&DST_SE(NVM_log), tempSE);
  ST_WB_FLOAT(&DST_SW(NVM_log), tempSW);
  ST_WB_FLOAT(&DST_NT(NVM_log), tempNT);
  ST_WB_FLOAT(&DST_NB(NVM_log), tempNB);
  ST_WB_FLOAT(&DST_ST(NVM_log), tempST);
  ST_WB_FLOAT(&DST_SB(NVM_log), tempSB);
  ST_WB_FLOAT(&DST_ET(NVM_log), tempET);
  ST_WB_FLOAT(&DST_EB(NVM_log), tempEB);
  ST_WB_FLOAT(&DST_WT(NVM_log), tempWT);
  ST_WB_FLOAT(&DST_WB(NVM_log), tempWB);
  CLFLUSH(&DST_C(NVM_log));
  CLFLUSH(&DST_N(NVM_log));
  CLFLUSH(&DST_S(NVM_log));
  CLFLUSH(&DST_E(NVM_log));
  CLFLUSH(&DST_W(NVM_log));
  CLFLUSH(&DST_T(NVM_log));
  CLFLUSH(&DST_B(NVM_log));
  CLFLUSH(&DST_NE(NVM_log));
  CLFLUSH(&DST_NW(NVM_log));
  CLFLUSH(&DST_SE(NVM_log));
  CLFLUSH(&DST_SW(NVM_log));
  CLFLUSH(&DST_NT(NVM_log));
  CLFLUSH(&DST_NB(NVM_log));
  CLFLUSH(&DST_ST(NVM_log));
  CLFLUSH(&DST_SB(NVM_log));
  CLFLUSH(&DST_ET(NVM_log));
  CLFLUSH(&DST_EB(NVM_log));
  CLFLUSH(&DST_WT(NVM_log));
  CLFLUSH(&DST_WB(NVM_log));
